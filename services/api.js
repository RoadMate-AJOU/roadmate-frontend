...
export const authService = {
  signup: async (username, password, name) => {
    ...
    return await handleApiResponse(response); // { id, name }
  },
  ...
  login: async (username, password) => {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      debugLog('LOGIN_ERROR', '❌ 로그인 실패', { errorText });
      throw new Error(`로그인 실패: ${errorText}`);
    }

    const token = await response.text(); // 텍스트 응답
    debugLog('LOGIN_SUCCESS', '✅ 로그인 성공, 토큰 수신', { token });
    return token;
  },
